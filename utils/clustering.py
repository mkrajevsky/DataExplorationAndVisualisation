...
def cluster_dataset_(X, nclusters, algorithm, alg_name):
...
def cluster_datasets_(datasets, algorithm, alg_name):
    ...
    for (battery, dataset) in  tqdm.tqdm(datasets.items(), desc=f"Clustering datasets with {alg_name}"):
        results[battery] = cluster_dataset_(X=dataset['X'], nclusters=dataset['n_clusters'], algorithm=algorithm, alg_name=alg_name)
    return results